...
### Fixed
- More granular diagnostics inside SessionManager.initialize()

## [0.4.13] - 2026-02-01

### Fixed
- Added more granular diagnostic logging to pinpoint startup hang

## [0.4.12] - 2026-02-01

### Added
- **Diagnostic logging**: Temporary startup diagnostics that write to `~/.kata-agents/startup-debug.log` on every launch
- Logs critical values: `process.execPath`, `app.isPackaged`, `isPackagedApp()` result, workspace/session info
- Helps diagnose why production builds behave differently from local builds

## [0.4.11] - 2026-02-01

### Fixed
- **CRITICAL**: Fix `isPackagedApp()` using `app.getAppPath()` before app is ready
- The function is called at module load time (before `app.whenReady()`), but `app.getAppPath()` returns incorrect values until the app is fully initialized
- Now uses only `process.execPath` which is reliable at any point in the app lifecycle
- This fixes the "spawn bun ENOENT" error that still occurred in v0.4.10 CI builds
...
### Fixed
- Update copyright in About dialog from "Craft Docs Ltd." to "Kata Agents Contributors"

## [0.4.7] - 2026-01-31

### Fixed
- **CRITICAL**: Fix arm64 DMG containing x64 binary - electron-builder.yml was specifying both architectures which overrode the CLI `--arch` flag, causing each job to build both archs and contaminate releases
- Remove explicit arch list from electron-builder.yml so CLI flag controls architecture
