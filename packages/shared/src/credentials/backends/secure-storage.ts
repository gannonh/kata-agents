 ...
 *
 * Stores credentials in an encrypted file at ~/.kata-agents/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.kata-agents');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('kata-agent-v2') // Bumped version for new key derivation
      .digest();
