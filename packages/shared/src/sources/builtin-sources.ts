 ...
 *
 * NOTE: kata-agents-docs is now an always-available MCP server configured
 * directly in kata-agent.ts, not a source. This file is kept for backwards
 ...
 *
 * Currently returns empty array - kata-agents-docs has been moved to
 * an always-available MCP server in kata-agent.ts.
 *
...
/**
 * Get the built-in Kata Agents docs source.
 *
 * @deprecated kata-agents-docs is now an always-available MCP server
 * configured directly in kata-agent.ts. This function is kept for
...
export function getDocsSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const placeholderConfig: FolderSourceConfig = {
    id: 'builtin-kata-agents-docs',
    name: 'Kata Agents Docs',
    slug: 'kata-agents-docs',
    enabled: false,
    provider: 'mintlify',
    type: 'mcp',
    mcp: {
      transport: 'http',
      url: 'https://agents.kata.sh/docs/mcp',
      authType: 'none',
    },
    tagline: 'Search Kata Agents documentation and source setup guides',
    icon: '📚',
 ...
 *
 * Returns false - kata-agents-docs is now an always-available MCP server,
