 ...
 * Default location: $KATA_CONFIG_DIR/workspaces/ (or ~/.kata-agents/workspaces/)
 */
...
/**
 * Get the default workspaces directory under the active config directory.
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.kata-agents/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.kata-agents/workspaces/
 */
 ...
 * Resolution order: ENV (KATA_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.KATA_LOCAL_MCP_ENABLED;
