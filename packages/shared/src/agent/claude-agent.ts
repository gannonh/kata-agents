...
// Plan types are used by UI components; not needed in kata-agent.ts since Safe Mode is user-controlled
...
// Documentation is served via local files at ~/.kata-agents/docs/
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      const fullMcpServers: Options['mcpServers'] = {
        ...
        // Kata Agents documentation - always available for searching setup guides
      ...
      // Gated by enable1MContext in global config (~/.kata-agents/config.json).
      ...
      const options: Options = {
        ...
        hooks: (() => {
          ...
          if (Object.keys(userHooks).length > 0) {
            debug('[KataAgent] User SDK hooks loaded:', Object.keys(userHooks).join(', '));
          }
          ...
          yield {
            ...
            error: isCwdCause ? {
              ...
              message:
                ...
                'or a security tool removed it). Reinstalling Kata Agents typically fixes this.',
              details: [
