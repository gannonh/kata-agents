...
// Mock filesystem checks for guide paths while keeping config defaults self-contained.
const CONFIG_DEFAULTS_PATH = join(homedir(), '.kata-agents', 'config-defaults.json');
...
mock.module('node:fs', () => ({
  ...
  readFileSync: (path: string) => {
    if (path === CONFIG_DEFAULTS_PATH) {
      return JSON.stringify({
        defaults: { browserToolEnabled: true },
        workspaceDefaults: { permissionMode: 'ask' },
      });
    }
    return originalReadFileSync(path, 'utf-8');
  },
}));
...
function browserDocPath(): string {
  return resolve(join(homedir(), '.kata-agents', 'docs', 'browser-tools.md'));
}
...
describe('PrerequisiteManager', () => {
  ...
  describe('rule matching', () => {
    ...
    it('exempts kata-agents-docs MCP tools', () => {
      mockExistsPaths.add(guidePath('kata-agents-docs'));
      const result = manager.checkPrerequisites('mcp__kata-agents-docs__search');
      expect(result.allowed).toBe(true);
    });
