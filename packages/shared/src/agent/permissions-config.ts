 ...
 *
 * File locations:
 * - Workspace: ~/.kata-agents/workspaces/{slug}/permissions.json
 * - Per-source: ~/.kata-agents/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
...
function shouldCompileBashPattern(pattern: string): boolean {
  if (!FEATURE_FLAGS.kataAgentsCli && pattern.startsWith('^kata-agent\\s')) {
    return false;
  }
  return true;
}
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.kata-agents/permissions/default.json)
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping kata-agent bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }
