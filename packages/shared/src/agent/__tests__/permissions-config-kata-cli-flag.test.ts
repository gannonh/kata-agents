...
const originalCliFlag = process.env.KATA_FEATURE_KATA_AGENTS_CLI
    ...
    JSON.stringify(
      {
        version: '2026-03-07',
        allowedBashPatterns: [
          { pattern: '^kata-agent\\s+label\\s+list\\b', comment: 'kata-agent label read-only operations' },
...
afterEach(() => {
  ...
  if (originalCliFlag === undefined) delete process.env.KATA_FEATURE_KATA_AGENTS_CLI
  else process.env.KATA_FEATURE_KATA_AGENTS_CLI = originalCliFlag
})

describe('permissions config kata-agents-cli feature flag', () => {
  it('skips compiling kata-agent bash allowlist patterns when feature is disabled', () => {
    const tempConfigDir = mkdtempSync(join(tmpdir(), 'craft-permissions-'))
    try {
      process.env.KATA_CONFIG_DIR = tempConfigDir
      ...
      expect(sources.some(source => source.startsWith('^kata-agent\\s'))).toBe(false)
  ...
  it('compiles kata-agent bash allowlist patterns when feature is enabled', () => {
    const tempConfigDir = mkdtempSync(join(tmpdir(), 'craft-permissions-'))
    try {
      process.env.KATA_CONFIG_DIR = tempConfigDir
      ...
      const sources = merged.readOnlyBashPatterns.map(p => p.source)
      expect(sources).toContain('^kata-agent\\s+label\\s+list\\b')
      expect(sources).toContain('^rg\\b')
    } finally {
      rmSync(tempConfigDir, { recursive: true, force: true })
    }
