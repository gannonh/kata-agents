...
beforeAll(() => {
  origFlag = process.env.KATA_FEATURE_KATA_AGENTS_CLI
...
afterAll(() => {
  if (origFlag === undefined) {
    ...
    process.env.KATA_FEATURE_KATA_AGENTS_CLI = origFlag
  }
})
...
describe('PiAgent pre-tool labels guard', () => {
  it('blocks Read on workspace labels/config.json even when session workingDirectory is outside workspace root', async () => {
    ...
    expect(String(response?.reason ?? '')).toContain('kata-agent label --help')
