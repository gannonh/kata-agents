#!/usr/bin/env bun
/**
 * Debug script to print the full Kata Agent system prompt with annotations.
...
printAnnotation('  2. Kata Agent Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.kata-agents/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@kata-sh/electron/main.log' },
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.kata-agents/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by KataAgent.formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Kata Agent additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Kata Agent Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/kata-agent.ts       ${colors.dim}// User message building${colors.reset}
