...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in Kata Agents.
...
```
mcp__kata-agents-docs__SearchCraftAgents({ query: "{service} source setup" })
```
...
Agent: [FIRST: Searches for Linear guide]
       mcp__kata-agents-docs__SearchCraftAgents({ query: "linear source setup" })
...
`~/.kata-agents/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.kata-agents/workspaces/{ws}/sources/my-source
   ```
