...
import type { StoredAttachment, ContentBadge } from '@kata-sh/core'
import { normalizePath } from '@kata-sh/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .kata-agents workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.kata-agents\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
