...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to KATA_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.KATA_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.KATA_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.KATA_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute KATA_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute KATA_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (KATA_UV / KATA_NODE / KATA_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.KATA_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.KATA_UV, 'Python/uv')
        : process.env.KATA_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure KATA_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.KATA_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.KATA_NODE, 'Node')
        : process.env.KATA_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure KATA_NODE or install node on PATH.'
    );
  }

  if (process.env.KATA_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.KATA_BUN, 'Bun')
      : process.env.KATA_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure KATA_BUN or install bun on PATH.'
  );
}
