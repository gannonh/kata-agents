 ...
 * It uses the shared handlers from @kata-sh/session-tools-core to ensure
 ...
 *   --workspace-root: Path to workspace folder (~/.kata-agents/workspaces/{id})
...
import { isDeveloperFeedbackEnabled } from '@kata-sh/shared/feature-flags';
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.kata-agents/workspaces/{id}
      // preferencesPath = ~/.kata-agents/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.KATA_CONFIG_DIR || join(workspaceRootPath, '..', '..');
...
// ============================================================
// Kata Agents Docs Upstream Proxy
// ============================================================

const DOCS_MCP_URL = 'https://agents.kata.sh/docs/mcp';
...
/**
 * Connect to the kata-agents-docs MCP server and fetch its tool definitions.
...
async function connectDocsUpstream(): Promise<void> {
  try {
    const client = new Client(
      { name: 'kata-agent-session-proxy', version: '1.0.0' },
      { capabilities: {} }
    );
    ...
    console.error(`Kata Agents Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`Kata Agents Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`Kata Agents Docs server is not connected. Tool '${name}' unavailable.`);
  }
...
async function handleCallLlm(
  ...
  // Uses callbackPort from CLI arg (--callback-port) or env var (KATA_LLM_CALLBACK_PORT).
  ...
  return errorResponse(
    ...
    'HTTP callback (KATA_LLM_CALLBACK_PORT). Neither is available.'
  );
}
...
async function main() {
  ...
  const config: SessionConfig = {
    ...
    callbackPort: callbackPort || process.env.KATA_LLM_CALLBACK_PORT,
  };
