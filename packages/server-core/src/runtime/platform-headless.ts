...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (process.env.KATA_DEBUG === 'true' || process.env.KATA_IS_PACKAGED !== 'true') {
 ...
 * - KATA_APP_ROOT — override appRootPath (default: cwd)
 * - KATA_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
 * - KATA_IS_PACKAGED — 'true' for production (default: false)
 * - KATA_VERSION — app version string (default: '0.0.0-dev')
 * - KATA_DEBUG — 'true' to enable debug logging
 */
export function createHeadlessPlatform(options?: { appVersion?: string }): PlatformServices {
  const logger = createConsoleLogger()
  const isDebugMode = process.env.KATA_DEBUG === 'true' || process.env.KATA_IS_PACKAGED !== 'true'

  return {
    appRootPath: process.env.KATA_APP_ROOT || process.cwd(),
    resourcesPath: process.env.KATA_RESOURCES_PATH || join(process.cwd(), 'resources'),
    isPackaged: process.env.KATA_IS_PACKAGED === 'true',
    appVersion: process.env.KATA_VERSION || options?.appVersion || '0.0.0-dev',
