...
- If the app asks for a workspace, select one or enter a name and choose `Create workspace`.
...
- **Defer setup.** In the provider-selection view, click `[data-testid="onboarding-setup-later"]`. The result is either `#app-ready` or the workspace picker.
...
- Workspace picker creation is only needed when `#workspace-picker` appears; do not assume it is always shown.
