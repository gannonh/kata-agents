 ...
 * release. Ported from kata-code's resolve-nightly-release.ts, retargeted to
 * Bun and apps/electron/package.json.
 *
 * CLI:
 *   bun run scripts/release/resolve-nightly-release.ts \
 *     --date YYYYMMDD --run-number N --sha <sha> \
 *     [--root <dir>] [--github-output]
 *
 * Without --github-output it prints `key=value` lines to stdout; with it, it
 * appends them to $GITHUB_OUTPUT.
 */
...
export function resolveNightlyReleaseMetadata(
  ...
  return {
    ...
    version,
    tag: `v${version}`,
    name: `Kata Agents Nightly ${version} (${shortSha})`,
    shortSha,
  }
}
...
function readDesktopBaseVersion(rootDir: string): string {
  const pkgPath = resolve(rootDir, 'apps/electron/package.json')
...
function main(): void {
  ...
  if (args.githubOutput) {
    ...
    appendFileSync(githubOutput, entries.map(([k, v]) => `${k}=${v}\n`).join(''))
  } else {
