#!/usr/bin/env bash
# =============================================================================
# Docker smoke test for Kata Agents Server
#
...
docker run -d \
  --name "$CONTAINER_NAME" \
  -p "$PORT:9100" \
  -e "KATA_SERVER_TOKEN=$TOKEN" \
  -e "KATA_RPC_HOST=0.0.0.0" \
  -e "KATA_RPC_PORT=9100" \
  "$IMAGE"
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
  ...
  if docker logs "$CONTAINER_NAME" 2>&1 | grep -q "KATA_SERVER_URL="; then
    READY=true
    break
  fi
