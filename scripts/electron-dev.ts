...
// Detects instance number from folder name suffix (e.g., kata-agents-1 → instance 1)
function detectInstance(): void {
  // Don't override if already set (e.g., by sourcing detect-instance.sh first)
  if (process.env.KATA_VITE_PORT) return;
  ...
  if (match) {
    ...
    process.env.KATA_INSTANCE_NUMBER = instanceNum;
    process.env.KATA_VITE_PORT = `${instanceNum}173`;
    process.env.KATA_APP_NAME = `Kata Agents [${instanceNum}]`;
    process.env.KATA_CONFIG_DIR = join(process.env.HOME || "", `.kata-agents-${instanceNum}`);
    process.env.KATA_DEEPLINK_SCHEME = `kataagents${instanceNum}`;
    console.log(`🔢 Instance ${instanceNum} detected: port=${process.env.KATA_VITE_PORT}, config=${process.env.KATA_CONFIG_DIR}`);
  }
}
...
function getElectronEnv(): Record<string, string> {
  const vitePort = process.env.KATA_VITE_PORT || "5173";
  ...
  return {
    ...
    KATA_CONFIG_DIR: process.env.KATA_CONFIG_DIR || "",
    KATA_APP_NAME: process.env.KATA_APP_NAME || "Kata Agents",
    KATA_DEEPLINK_SCHEME: process.env.KATA_DEEPLINK_SCHEME || "kataagents",
    KATA_INSTANCE_NUMBER: process.env.KATA_INSTANCE_NUMBER || "",
  };
}
