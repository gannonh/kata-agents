...
async function buildWhatsAppWorker(): Promise<void> {
  ...
  const proc = spawn({
    cmd: [
      ...
      // call site and not used by Kata Agent (we send text + documents, no
...
async function main(): Promise<void> {
  ...
  const proc = spawn({
    cmd: [
      ...
      // Cookie import opens Chromium Cookies DBs via node:sqlite (Electron/Node)
      // or bun:sqlite (unit tests). These are runtime builtins, not npm packages.
