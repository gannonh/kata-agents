...
test.describe(`Managed worktree Git flow ${E2E_TAGS.git}`, () => {
  test("prepares, reviews, commits, and removes a managed worktree", async ({
    ...
    testInfo.annotations.push({
      type: "validation",
      description:
        "Real Electron + real Git repository with a non-GitHub remote; GitHub controls stay hidden.",
    });
    ...
    try {
      ...
      expect(prepared.checkout.expectedBranch).toMatch(
        /^kata-agent\/[0-9a-f]{8}$/,
      );
      ...
      await expect(changesAffordance).toContainText("1 file", {
        timeout: 7_000,
      });

      const headBeforeGuidance = await git(
      ...
      expect(
        await git(prepared.checkout.checkoutPath, "rev-parse", "HEAD"),
      ).toBe(headBeforeGuidance);
      ...
      await changesPanel
        .locator('[data-testid="git-change-file"][data-git-path="README.md"]')
        .click();
      await expect(
        changesPanel.getByText("Validated through the Changes panel."),
      ).toBeVisible();
      await attachScreenshot(
        page,
        testInfo,
        "02-changes-review-and-github-guidance",
      );
      ...
      ).toBe("test(git): validate managed worktree flow");
      expect(
        await git(prepared.checkout.checkoutPath, "status", "--porcelain"),
      ).toBe("");
      ...
      await page.evaluate(async (sessionId) => {
        ...
        await api.sessionCommand(sessionId, {
          type: "rename",
          name: "Git Workspace UAT",
        });
      }, prepared.id);

      const titleButton = page
        .getByRole("button", { name: /Git Workspace UAT/ })
        .first();
      ...
      const destructiveWarning = page.getByTestId(
        "git-delete-destructive-warning",
      );
      await expect(destructiveWarning).toContainText("1 unpushed commit");
      await attachScreenshot(
        page,
        testInfo,
        "04-destructive-removal-confirmation",
      );
      ...
      expect(
        await git(
          ...
          "branch",
          "--list",
          prepared.checkout.expectedBranch ?? "",
        ),
