...
`.github/workflows/release.yml`. The workflow has two triggers:
- **`workflow_dispatch`** — manual dispatch with `channel` (stable|nightly) and `dry_run` (true|false)
- **Tag push** — pushing a `v*.*.*` tag triggers a stable release automatically
...
# Check current version in electron package
...
For **stable dispatch**, the version used is whatever is in `apps/electron/package.json`.
Make sure it's been bumped to the intended release version before dispatching.

For **nightly dispatch**, the version is computed automatically as
...
The workflow has four jobs: `release_meta` → `signing_gate` → `build` (matrix) → `release`.
...
2. For stable releases: bump the version in `apps/electron/package.json` and `package.json`
   for the next development cycle and commit to main
