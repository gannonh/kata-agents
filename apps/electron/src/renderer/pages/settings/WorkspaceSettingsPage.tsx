...
export default function WorkspaceSettingsPage() {
  ...
  const [wsThinkingLevel, setWsThinkingLevel] = useState<ThinkingLevel>(
    DEFAULT_THINKING_LEVEL,
  );
  ...
  // Mode cycling state
  const [enabledModes, setEnabledModes] = useState<PermissionMode[]>([
    "safe",
    "ask",
    "allow-all",
  ]);
  ...
  useEffect(() => {
    const loadWorkspaceSettings = async () => {
      ...
      try {
        const settings =
          await window.electronAPI.getWorkspaceSettings(activeWorkspaceId);
        if (settings) {
          setWsName(settings.name || "");
          setWsNameEditing(settings.name || "");
          setWsModel(settings.model || "claude-sonnet-4-5");
    ...
    return (
      <div className="h-full flex flex-col">
        <PanelHeader
          ...
          actions={
            <HeaderMenu
              route={routes.view.settings("workspace")}
              helpFeature="workspaces"
            />
          }
        />
              ...
              <SettingsSection title="Permissions">
                <SettingsCard>
                  <SettingsMenuSelectRow
                    ...
                    onValueChange={(v) =>
                      handlePermissionModeChange(v as PermissionMode)
                    }
                    options={[
                      {
                        value: "safe",
                        label: PERMISSION_MODE_CONFIG["safe"].shortName,
                        description: "Read-only, no changes allowed",
                      },
                      {
                        value: "ask",
                        label: PERMISSION_MODE_CONFIG["ask"].shortName,
                        description: "Prompts before making edits",
                      },
                      {
                        value: "allow-all",
                        label: PERMISSION_MODE_CONFIG["allow-all"].shortName,
