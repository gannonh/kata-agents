...
import { Spinner, FullscreenOverlayBase, Tooltip, TooltipTrigger, TooltipContent } from '@kata-sh/ui'
    ...
    switch (provider) {
      ...
      case 'pi': {
        ...
        const piLabel = !isSubscription && connection.piAuthProvider
          ...
          : null
        parts.push(piLabel ?? 'Kata Agents Backend')
        break
      }
      case 'pi_compat':
        parts.push(connection.baseUrl?.toLowerCase().includes('manifest.build')
          ? 'Manifest'
          : 'Kata Agents Backend Compatible')
        break
                ...
                options={[
                  ...
                  ...llmConnections.map((conn) => ({
                    ...
                    description: conn.providerType === 'anthropic' ? 'Anthropic' :
                                 conn.providerType === 'pi' ? 'Kata Agents Backend' :
                  ...
                  <SettingsMenuSelectRow
                    ...
                    options={llmConnections.map((conn) => ({
                      ...
                      description: conn.providerType === 'anthropic' ? 'Anthropic API' :
                                   conn.providerType === 'pi' ? 'Kata Agents Backend' :
                                   conn.providerType === 'pi_compat' ? (conn.baseUrl?.toLowerCase().includes('manifest.build') ? 'Manifest' : 'Kata Agents Backend Compatible') :
                                   conn.providerType || 'Unknown',
                    }))}
                  />
                  <SettingsMenuSelectRow
