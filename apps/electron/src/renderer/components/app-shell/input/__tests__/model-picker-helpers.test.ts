...
import type { LlmConnection } from '@kata-sh/shared/config/llm-connections'
import {
  formatTokenCount,
...
describe('groupConnectionsByProvider', () => {
  ...
  test('places "Anthropic" group before pi groups (display order)', () => {
    ...
    expect(result.map(([k]) => k)).toEqual(['Anthropic', 'Kata Agents Backend'])
  })
