...
export function formatBrowserAnnotationsAsMarkdown(annotations: BrowserPageAnnotation[]): string {
  ...
  const urls = new Set(annotations.map((annotation) => annotation.payload.page.sanitizedUrl))
  const sharedUrl = urls.size === 1
  const first = firstAnnotation.payload
  const lines: string[] = [
    sharedUrl ? `## Page feedback: ${formatPageHeading(first)}` : '## Page feedback',
    '',
    `**Browser instance id:** ${firstAnnotation.browserPageId}`,
