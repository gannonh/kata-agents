...
afterEach(() => {
  // spies restored per-test via mockRestore
})
...
describe('formatBrowserAnnotationsAsMarkdown', () => {
  it('includes agent-useful selectors, source, react tree, styles, and feedback', () => {
    ...
    expect(markdown).toContain('## Page feedback: /pricing')
    ...
    expect(markdown).not.toMatch(/\*\*URL:\*\* https:\/\/example.com\/pricing\n\*\*Browser instance id:/)
  })
  ...
  it('prints each annotation URL so notes from later pages are not relabeled', () => {
    const first = makeAnnotation()
    const second = makeAnnotation({
      id: 'annotation-2',
      comment: 'The checkout heading is too small.',
      payload: {
        ...first.payload,
        page: {
          ...first.payload.page,
          sanitizedUrl: 'https://example.com/checkout',
        },
      },
    })
    ...
    expect(markdown).toContain('**URL:** https://example.com/pricing')
    expect(markdown).toContain('**URL:** https://example.com/checkout')
    expect(markdown).not.toContain('## Page feedback: /pricing')
  })
  ...
  it('bounds large inline page text without regex replacement passes', () => {
    const replaceSpy = spyOn(String.prototype, 'replace')
    const annotation = makeAnnotation()
