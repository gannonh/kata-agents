 ...
 * Deep Link Handler
 *
 * Parses kataagents:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   kataagents://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   kataagents://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   kataagents://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   kataagents://sources[/source/{sourceSlug}]          - Sources list
 *   kataagents://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   kataagents://action/{actionName}[/{id}][?params]
 *   kataagents://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *
 * Examples:
 *   kataagents://allSessions                               (all sessions view)
 *   kataagents://allSessions/session/abc123                (specific session)
 *   kataagents://settings/shortcuts                     (shortcuts page)
 *   kataagents://sources/source/github                  (github source info)
 *   kataagents://action/new-chat                        (uses active window)
 *   kataagents://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   kataagents://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== 'kataagents:') {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., kataagents://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., kataagents://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // kataagents://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // kataagents://allSessions/..., kataagents://settings/..., etc. (compound routes)
    ...
    // kataagents://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // kataagents://action/... (no workspace - uses active window)
    if (host === 'action') {
