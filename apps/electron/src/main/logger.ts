...
import type {
  ...
  MessagingLogMeta,
  MessagingLogger,
} from '@kata-sh/messaging-gateway'
 ...
 * 2) KATA_IS_PACKAGED env (when explicitly set)
...
function resolveDebugMode(): boolean {
  ...
  const packagedEnv = process.env.KATA_IS_PACKAGED
 ...
 */
export const messagingGatewayLogPath = join(homedir(), '.kata-agents', 'logs', 'messaging-gateway.log')
