...
import { setupI18n, i18n, SUPPORTED_LANGUAGE_CODES, type LanguageCode } from '@kata-sh/shared/i18n'
import { getPersistedUiLanguage, setPersistedUiLanguage } from '@kata-sh/shared/config'
setupI18n()
...
import { bootstrapServer, releaseServerLock } from '@kata-sh/server-core/bootstrap'
...
import { initModelRefreshService, getModelRefreshService, setFetcherPlatform } from '@kata-sh/server-core/model-fetchers'
import { setSearchPlatform, setImageProcessor } from '@kata-sh/server-core/services'
import { createApplicationMenu } from './menu'
import { WindowManager } from './window-manager'
import { loadWindowState, saveWindowState } from './window-state'
import { getWorkspaces, getWorkspaceByNameOrId, loadStoredConfig, addWorkspace, saveConfig } from '@kata-sh/shared/config'
import { getDefaultWorkspacesDir } from '@kata-sh/shared/workspaces'
import { initializeDocs } from '@kata-sh/shared/docs'
import { initializeReleaseNotes } from '@kata-sh/shared/release-notes'
import { ensureDefaultPermissions } from '@kata-sh/shared/agent/permissions-config'
import { ensureToolIcons, ensurePresetThemes } from '@kata-sh/shared/config'
import { setBundledAssetsRoot } from '@kata-sh/shared/utils'
import { initializeBackendHostRuntime } from '@kata-sh/shared/agent/backend'
import { setPowerShellValidatorRoot } from '@kata-sh/shared/agent'
import { handleDeepLink } from './deep-link'
import { BrowserPaneManager } from './browser-pane-manager'
import { OAuthFlowStore } from '@kata-sh/shared/auth'
...
import { setPerfEnabled, enableDebug } from '@kata-sh/shared/utils'
import { registerPiModelResolver } from '@kata-sh/shared/config'
import { getPiModelsForAuthProvider, getAllPiModels } from '@kata-sh/shared/config'
...
// These are available to all agent Bash sessions via KATA_UV, KATA_SCRIPTS env vars
// and PATH prepend. uv auto-downloads Python 3.12 on first use (~5s, then cached).
{
  ...
  process.env.KATA_IS_PACKAGED = app.isPackaged ? '1' : '0'
  process.env.KATA_RESOURCES_BASE = resourcesBase
  process.env.KATA_APP_ROOT = app.isPackaged ? app.getAppPath() : process.cwd()

  process.env.KATA_UV = bundledUvExists ? uvBinary : (fallbackUv ?? uvBinary)
  ...
  if (existsSync(bunBinary)) {
    process.env.KATA_BUN = bunBinary
  }
  ...
  if (!bundledUvExists) {
    mainLog.warn('Bundled uv binary missing, CLI document tools may fail unless uv is available on PATH.', {
      ...
      usingCraftUv: process.env.KATA_UV,
    })
  }

  if (isDebugMode) {
    mainLog.info('CLI tools configured:', { uvBinary: process.env.KATA_UV, binDir, scriptsDir, bundledUvExists })
  }
}
...
// Custom URL scheme for deeplinks (e.g., kataagents://auth-complete)
// Supports multi-instance dev: KATA_DEEPLINK_SCHEME env var (kataagents1, kataagents2, etc.)
const DEEPLINK_SCHEME = process.env.KATA_DEEPLINK_SCHEME || 'kataagents'
...
// Supports multi-instance dev: KATA_APP_NAME env var (e.g., "Kata Agents [1]")
app.setName(process.env.KATA_APP_NAME || 'Kata Agents')

// Register as default protocol client for kataagents:// URLs
...
// Only bypasses cert validation for the exact KATA_SERVER_URL origin — all other connections
...
if (process.env.KATA_SERVER_URL) {
  let serverOrigin: string | undefined
  try {
    serverOrigin = normalizeOriginForCert(process.env.KATA_SERVER_URL)
  } catch {
...
app.whenReady().then(async () => {
  ...
  process.env.KATA_IS_PACKAGED = app.isPackaged ? 'true' : 'false'
  ...
  // Seed preset themes to ~/.kata-agents/themes/ (copies bundled theme JSONs on first run)
  ensurePresetThemes()
  ...
  if (process.platform === 'darwin' && app.dock) {
    ...
    // KATA_INSTANCE_NUMBER is set by detect-instance.sh for numbered folders
    const instanceNum = process.env.KATA_INSTANCE_NUMBER
    if (instanceNum) {
  ...
  try {
    ...
    // When KATA_SERVER_URL is set, this Electron instance is a thin client —
    ...
    const isClientOnly = !!process.env.KATA_SERVER_URL
    const isHeadless = !!process.env.KATA_HEADLESS

    if (isClientOnly) {
      mainLog.info(`Client-only mode: KATA_SERVER_URL=${process.env.KATA_SERVER_URL} (server initialization skipped)`)
    }
    ...
    if (!isClientOnly) {
      ...
      if (process.platform === 'win32') {
        const { getGitBashPath, clearGitBashPath } = await import('@kata-sh/shared/config')
      ...
      if (process.platform === 'win32') {
        ...
        if (!vcCheck.installed) {
          ...
          process.env.KATA_VCREDIST_MISSING = '1'
          if (vcCheck.downloadUrl) {
            process.env.KATA_VCREDIST_URL = vcCheck.downloadUrl
          }
        } else if (isDebugMode) {
      ...
      const { getServerConfig } = await import('@kata-sh/shared/config')
      ...
      const serverToken = serverModeEnabled && embeddedServerConfig.token
        ...
        : randomUUID()
      const rpcHost = process.env.KATA_RPC_HOST
        ?? (serverModeEnabled ? '0.0.0.0' : '127.0.0.1')
      const rpcPort = process.env.KATA_RPC_PORT
        ? parseInt(process.env.KATA_RPC_PORT, 10)
        : (serverModeEnabled ? embeddedServerConfig.port : 0)

      // Load TLS certificates if configured
      let tls: import('@kata-sh/server-core/transport').WsRpcTlsOptions | undefined
      ...
      const instance = await bootstrapServer<SessionManager, HandlerDeps>({
        ...
        createHandlerDeps: ({ sessionManager: sm, platform: p, oauthFlowStore: ofs }) => {
          ...
          messagingHandle = createMessagingBootstrap({
            ...
            // at ~/.kata-agents/logs/messaging-gateway.log.
        ...
        initModelRefreshService: () => initModelRefreshService(async (slug: string) => {
          const { getCredentialManager } = await import('@kata-sh/shared/credentials')
      ...
      ipcMain.handle('workspace:remove', async (_event, workspaceId: string) => {
        const { removeWorkspace: remove } = await import('@kata-sh/shared/config')
      ...
      instance.wsServer.handle(RPC_CHANNELS.settings.GET_SERVER_CONFIG, async () => {
        const { getServerConfig: getConfig } = await import('@kata-sh/shared/config')
        return getConfig()
      })

      instance.wsServer.handle(RPC_CHANNELS.settings.SET_SERVER_CONFIG, async (_ctx: unknown, config: unknown) => {
        const { setServerConfig: setConfig } = await import('@kata-sh/shared/config')
        const cfg = config as import('@kata-sh/shared/config/server-config').ServerConfig
        // Validate port range
    ...
    try {
      const { getLlmConnection, getDefaultLlmConnection } = await import('@kata-sh/shared/config')
...
app.on('window-all-closed', () => {
  if (process.env.KATA_HEADLESS) return  // headless server stays alive
