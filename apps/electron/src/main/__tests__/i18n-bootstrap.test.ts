 ...
 * subprocess with `KATA_CONFIG_DIR` set in its env (same pattern as
    ...
    try {
      const r = runScript(
        ...
        `
          ...
          import { setPersistedUiLanguage, getPersistedUiLanguage } from '@kata-sh/shared/config';
          setupI18n();
  ...
  it('returns undefined when no language is persisted (no hydration step)', () => {
    ...
    try {
      const r = runScript(
        ...
        `
          import { setupI18n, i18n } from '@kata-sh/shared/i18n';
          import { getPersistedUiLanguage } from '@kata-sh/shared/config';
          setupI18n();
          const persisted = getPersistedUiLanguage();
