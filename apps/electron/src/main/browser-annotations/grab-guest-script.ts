...
// arm: install the overlay + hover tracking; state lives on window.__kataGrab so finalize/teardown can reach it.
const ARM_SCRIPT = `(function() {
  'use strict';

  // Why: always tear down any pre-existing state before arming. A malicious
  // guest page could predefine window.__kataGrab with a fake extractPayload
  // function. By tearing down unconditionally we ensure our freshly installed
  // extraction logic is the only code that runs.
  if (window.__kataGrab) {
    try {
      if (typeof window.__kataGrab.cleanup === 'function') {
        window.__kataGrab.cleanup();
      }
    } catch(e) {}
    delete window.__kataGrab;
  }

  // --- Budget constants (injected from shared protocol types) ---
  var BUDGET = ${JSON.stringify(GRAB_BUDGET)};
  var TEXT_NODE_SCAN_LIMIT = 80;
  var NEARBY_ELEMENT_SCAN_LIMIT = 80;

  var SAFE_ATTRS = new Set(${JSON.stringify([...GRAB_SAFE_ATTRIBUTE_NAMES])});
  var SECRET_PATTERNS = ${JSON.stringify([...GRAB_SECRET_PATTERNS])};
  var SAFE_URL_PROTOCOLS = new Set(['http:', 'https:', 'file:']);
  var STYLE_PROPS = ${JSON.stringify([...GRAB_STYLE_PROPERTIES])};
  ...
  function getHtmlSnippet(el) {
    ...
    var sensitive = clone.querySelectorAll('input[type="password"], input[type="hidden"]');
    for (var s = 0; s < sensitive.length; s++) {
      if (sensitive[s].hasAttribute('value')) {
        sensitive[s].setAttribute('value', '[redacted]');
      }
    }
    var html = clone.outerHTML || '';
    if (containsSecret(html)) return '[redacted]';
  ...
  function getSafeAttributes(el) {
    ...
    for (var i = 0; i < el.attributes.length; i++) {
      if (kept >= BUDGET.attributesMaxCount) break;
...
const AWAIT_CLICK_SCRIPT = `(async function() {
  ...
  return await new Promise(function(resolve, reject) {
    ...
    function onContext(e) {
      ...
      if (!payload) return;
      grab.freezeHighlight();
      resolve({ __kataContextMenu: true, payload: payload });
    }
...
const TEARDOWN_SCRIPT = `(function() {
  ...
  // it with the __kataCancelled marker so the executeJavaScript call in main
