...
import { GRAB_BUDGET } from '@kata-sh/shared/protocol'
...
describe('buildGuestOverlayScript', () => {
  ...
  it('arm script contains shadow DOM setup', () => {
    ...
    expect(script).toContain('__kata-grab-host')
    expect(script).toContain('__kataGrab')
  })

  it('arm script contains budget constants matching shared types', () => {
    const script = buildGuestOverlayScript('arm')
    expect(script).toContain(`"textSnippetMaxLength":${GRAB_BUDGET.textSnippetMaxLength}`)
    expect(script).toContain(`"nearbyTextMaxEntries":${GRAB_BUDGET.nearbyTextMaxEntries}`)
    expect(script).toContain(`"htmlSnippetMaxLength":${GRAB_BUDGET.htmlSnippetMaxLength}`)
    expect(script).toContain(`"attributesMaxCount":${GRAB_BUDGET.attributesMaxCount}`)
  })
  ...
  it('arm script redacts password and hidden input values in HTML snippets', () => {
    const script = buildGuestOverlayScript('arm')
    expect(script).toContain('input[type="password"]')
    expect(script).toContain('input[type="hidden"]')
    expect(script).toContain("containsSecret(html)")
  })
  ...
  it('teardown script cancels pending awaitClick', () => {
    ...
    expect(buildGuestOverlayScript('awaitClick')).toContain('__kataCancelled')
  })
...
describe('awaitClick under a Zone.js-patched global Promise', () => {
  ...
  function armGrabHarness(options?: {
    ...
    window: { __kataGrab: Record<string, unknown> }
    click: () => void
    contextmenu: () => void
    cancel: () => void
  } {
    ...
    const window = { __kataGrab: grab }
    return {
      window,
      click: () => handlers.click?.(noopEvent),
      contextmenu: () => handlers.contextmenu?.(noopEvent),
      // cancelAwait is installed on __kataGrab by the script itself at runtime.
      cancel: () => (window.__kataGrab.cancelAwait as (() => void) | undefined)?.()
    }
  }
  ...
  it('resolves the context-menu marker across the boundary despite ZoneAwarePromise', async () => {
    ...
    expect(received).toHaveProperty('__kataContextMenu', true)
  ...
  it('resolves the teardown cancel marker across the boundary despite ZoneAwarePromise', async () => {
    ...
    expect(received).toEqual({ __kataCancelled: true })
  })
