...
import { scheduleChannelConfigDelivery, deliverChannelConfigs } from './channel-config-delivery'
...
export function registerIpcHandlers(sessionManager: SessionManager, windowManager: WindowManager, daemonManager?: DaemonManager | null): void {
  ...
  ipcMain.handle(IPC_CHANNELS.LOGOUT, async () => {
    try {
      ...
      const configPath = join(homedir(), '.kata-agents', 'config.json')
  ...
  ipcMain.handle(IPC_CHANNELS.SETTINGS_TEST_API_CONNECTION, async (_event, apiKey: string, baseUrl?: string, modelName?: string): Promise<{ success: boolean; error?: string; modelCount?: number }> => {
    ...
    try {
      ...
      if (isToolSupportError) {
        ...
        return { success: false, error: `Model "${displayModel}" does not support tool/function calling. Kata Agents requires a model with tool support (e.g. Claude, GPT-4, Gemini).` }
      }
