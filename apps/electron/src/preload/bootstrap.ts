 ...
 *
 * Thin-client mode (KATA_SERVER_URL):
...
import { createCallbackServer } from '@kata-sh/shared/auth/callback-server'
import { CHATGPT_OAUTH_CONFIG } from '@kata-sh/shared/auth/chatgpt-oauth-config'
import {
...
import type { ConfirmDialogSpec, FileDialogSpec, BrowserCapabilityRequest } from '@kata-sh/server-core/transport'
...
if (isClientOnly) {
  ...
  const wsUrl = process.env.KATA_SERVER_URL!
  const wsToken = process.env.KATA_SERVER_TOKEN ?? ''
  ...
  if (parsed.protocol === 'ws:' && !isLocalhost) {
    throw new Error(
      ...
      `Set KATA_RPC_TLS_CERT/KEY on the server to enable TLS.`
    )
  }

  // Workspace ID is optional — if missing, renderer shows a workspace picker
  const workspaceId = process.env.KATA_WORKSPACE_ID || ipcRenderer.sendSync('__get-workspace-id') || undefined
...
;(api as ElectronAPI).getSystemWarnings = async () => ({
  vcredistMissing: process.env.KATA_VCREDIST_MISSING === '1',
  downloadUrl: process.env.KATA_VCREDIST_URL,
})
