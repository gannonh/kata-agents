...
Write-Host "=== Building Kata Agents Windows Installer using electron-builder ===" -ForegroundColor Cyan
...
while (-not $builderSuccess -and $builderRetry -lt $maxBuilderRetries) {
    ...
    npx electron-builder --win --x64 @ConfigArgs --publish never 2>&1 | Tee-Object -Variable builderOutput
