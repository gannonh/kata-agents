...
**Never create `{version}.md` files in feature commits.** Versioned files in `release-notes/` are generated by the release workflow — `scripts/release/promote-release-notes.ts` consolidates pending entries into `{version}.md` at build time and, for stable releases, commits the promotion and resets the scratch file from the `finalize` job. See [`docs/operations/release.md`](../../../docs/operations/release.md#release-notes).
