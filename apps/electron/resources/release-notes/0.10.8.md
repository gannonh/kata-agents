...
- **Remote server settings** — The Server settings page is now available by default under Settings → Server. Enable Server Mode to accept remote client connections, configure the port, and optionally configure TLS. Set `KATA_FEATURE_EMBEDDED_SERVER=0` to hide the page.
