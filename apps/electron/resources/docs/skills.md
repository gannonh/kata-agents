...
This guide explains how to create and configure skills in Kata Agent.

> **CLI-first workflow (recommended):** Use `kata-agent skill ...` commands instead of editing `SKILL.md` files directly.
...
Kata Agent uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in Kata Agent
...
1. **Workspace skill checked first** - If `~/.kata-agents/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.kata-agents/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.kata-agents/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.kata-agents/workspaces/{ws}/skills/commit/SKILL.md`
