...
Automations allow you to trigger actions automatically when specific events occur in Kata Agent. You can:
...
### App Events (triggered by Kata Agent)
...
Send a prompt to Kata Agent (creates a new session for scheduled prompts).
...
- Environment variables are expanded (e.g., `$KATA_LABEL`)
...
{
  "type": "webhook",
  "url": "https://hooks.slack.com/services/${KATA_WH_SLACK_PATH}",
  "method": "POST",
  "body": {
    "text": "Session ${KATA_SESSION_NAME} status changed to ${KATA_NEW_STATE}"
  }
}
```
...
```json
{
  ...
  "auth": {
    "type": "bearer",
    "token": "${KATA_WH_API_TOKEN}"
  },
  "body": { "event": "$KATA_EVENT" }
}
```
...
```json
{
  ...
  "auth": {
    "type": "basic",
    "username": "${KATA_WH_USER}",
    "password": "${KATA_WH_PASS}"
  }
}
```
...
**Security:** Webhook actions only have access to `KATA_*` system variables and `KATA_WH_*` user-defined secrets. They do **not** have access to your full system environment (e.g., `$HOME`, `$PATH`, or other process variables).

## Environment Variables

Both prompt and webhook actions support variable expansion using `$VAR` or `${VAR}` syntax.

### System Variables (KATA_*)

These are automatically set by the automation system based on the triggering event:

| Variable | Description | Available For |
|----------|-------------|---------------|
| `$KATA_EVENT` | Event name (e.g., `LabelAdd`) | All events |
| `$KATA_EVENT_DATA` | Full event payload as JSON | All events |
| `$KATA_SESSION_ID` | Session ID | Events with session context |
| `$KATA_SESSION_NAME` | Session name | Events with session context |
| `$KATA_WORKSPACE_ID` | Workspace ID | All events |

**Per-event variables:**

| Event | Variable | Description |
|-------|----------|-------------|
| `LabelAdd` / `LabelRemove` | `$KATA_LABEL` | The label that was added/removed |
| `PermissionModeChange` | `$KATA_OLD_MODE`, `$KATA_NEW_MODE` | Previous and new permission mode |
| `FlagChange` | `$KATA_IS_FLAGGED` | `true` or `false` |
| `SessionStatusChange` | `$KATA_OLD_STATE`, `$KATA_NEW_STATE` | Previous and new status |
| `SchedulerTick` | `$KATA_LOCAL_TIME`, `$KATA_LOCAL_DATE` | Current time (`14:30`) and date (`2026-03-09`) |

### User-Defined Webhook Secrets (KATA_WH_*)

For webhook actions, you can define your own secrets by setting environment variables with the `KATA_WH_` prefix in your shell profile (e.g., `~/.zshrc`, `~/.bashrc`):

```bash
# In your shell profile
export KATA_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
export KATA_WH_DISCORD_URL="https://discord.com/api/webhooks/123/abc"
export KATA_WH_API_TOKEN="your-secret-token"
```
...
{
  "type": "webhook",
  "url": "${KATA_WH_SLACK_URL}",
  "method": "POST",
  "body": { "text": "Hello from Kata Agent!" }
}
```
...
```json
{
  "type": "webhook",
  "url": "https://api.example.com/events",
  "headers": { "Authorization": "Bearer ${KATA_WH_API_TOKEN}" },
  "body": { "event": "${KATA_EVENT}", "session": "${KATA_SESSION_NAME}" }
}
```

This keeps secrets out of `automations.json` (which may be shared or committed to version control).

> **Note:** Only variables prefixed with `KATA_WH_` are injected into webhook actions. Other environment variables (like `$HOME` or `$DATABASE_URL`) are not accessible to webhooks.
...
4. **Pair the supergroup.** In Kata Agent: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.

Verify by checking the supergroup row in Settings shows the group title. If automation runs fail later, `~/.kata-agents/logs/messaging-gateway.log` will show `automation_topic_bind_failed` with the underlying Telegram error.
        ...
        "actions": [
          {
            ...
            "url": "${KATA_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":warning: Permission escalated from safe to allow-all in *${KATA_SESSION_NAME}*" }
          }
        ]
      }
    ]
...
```json
{
  ...
  "automations": {
    "LabelAdd": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $KATA_LABEL was added. Log this change with a timestamp." }
        ]
      }
    ],
    "LabelRemove": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $KATA_LABEL was removed. Log this change with a timestamp." }
        ]
      }
    ]
  }
...
Sends a Slack message when a session is marked as done. Requires `KATA_WH_SLACK_URL` in your shell profile.
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "body": {
              "text": ":white_check_mark: Session *${KATA_SESSION_NAME}* marked as done"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "url": "${KATA_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":rotating_light: Urgent label added to *${KATA_SESSION_NAME}*" }
          },
          {
            "type": "prompt",
        ...
        "actions": [
          {
            ...
            "body": {
              ...
              "client_id": "${KATA_WH_CLIENT_ID}",
              "client_secret": "${KATA_WH_CLIENT_SECRET}"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "headers": {
              "Authorization": "Bearer ${KATA_WH_API_TOKEN}",
              "X-Source": "kata-agents"
            },
            "body": {
              "event": "${KATA_EVENT}",
              "session_id": "${KATA_SESSION_ID}",
              "old_status": "${KATA_OLD_STATE}",
              "new_status": "${KATA_NEW_STATE}"
            }
          }
        ]
      }
...
Ask Kata Agent to validate your automations configuration:
...
2. **Check env vars** — Ensure `KATA_WH_*` variables are set in your shell profile and Kata Agent was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
...
5. **Keep secrets out of config** - Use `KATA_WH_*` env vars for webhook URLs and tokens instead of hardcoding them in automations.json
