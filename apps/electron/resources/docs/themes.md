...
This guide explains how to customize the visual theme of Kata Agent.

## Overview

Kata Agent uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.kata-agents/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.kata-agents/theme.json` - Override specific colors (app-level)
