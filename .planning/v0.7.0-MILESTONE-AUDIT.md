...
`PluginManager.initializeAll()` is defined but never called in `daemon/entry.ts`. `shutdownAll()` is properly called on reconfigure (line 139) and shutdown (line 254). Current built-in plugins have no initialization logic, but future plugins with async setup would need this hook.
