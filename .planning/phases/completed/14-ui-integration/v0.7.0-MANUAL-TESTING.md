...
A **source slug** is a URL-safe name you choose when creating the source. It serves as both the directory name and the identifier that channel configs reference. For example, if you name your source `my-slack-bot`, the channel config references it with `"sourceSlug": "my-slack-bot"`.

To set this up:

1. Pick a slug for your source (e.g., `my-slack-bot`) and create it:
   ```bash
   WORKSPACE_ROOT="<your-workspace-root-path>"
   SOURCE_SLUG="my-slack-bot"
...
2. Open Kata Agents and navigate to Settings for your workspace
3. The source "Slack Bot" appears in the sources list
4. Click it, then enter the `xoxb-` bot token in the credentials field
5. The token is stored encrypted at `~/.kata-agents/credentials.enc`

When the daemon starts, it reads the channel config, finds `credentials.sourceSlug: "my-slack-bot"`, looks up the encrypted token for that source, and passes it to the Slack adapter.
...
1. Create the auth directory and source:
   ```bash
   WORKSPACE_ROOT="<your-workspace-root-path>"
...
2. In the app, navigate to the source and enter the auth directory path as the credential: `~/.kata-agents/whatsapp-auth`
