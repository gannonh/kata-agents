...
Your workspace ID is a UUID. Find it by checking the app config:

```bash
cat ~/.kata-agents/config.json | grep -A3 '"workspaces"'
```

Look for the `"id"` field next to your workspace name. For example:
```json
{
  "name": "kata-agents",
  "id": "488ac60f-9b94-6119-b610-71faac90f5f7"
}
```

Use this ID in the paths below, replacing `<WORKSPACE_ID>`.
...
mkdir -p ~/.kata-agents/workspaces/$WORKSPACE_ID/channels/test-slack

cat > ~/.kata-agents/workspaces/$WORKSPACE_ID/channels/test-slack/config.json << 'EOF'
{
  "slug": "test-slack",
  "adapter": "slack",
  "enabled": true,
...
```bash
cat ~/.kata-agents/workspaces/$WORKSPACE_ID/channels/test-slack/config.json
```
...
- The directory is gone:
  ```bash
  ls ~/.kata-agents/workspaces/$WORKSPACE_ID/channels/test-slack/
...
mkdir -p ~/.kata-agents/workspaces/$WORKSPACE_ID/channels/slack-main

cat > ~/.kata-agents/workspaces/$WORKSPACE_ID/channels/slack-main/config.json << EOF
{
  "slug": "slack-main",
  "adapter": "slack",
  "enabled": true,
...
cat ~/.kata-agents/workspaces/$WORKSPACE_ID/config.json
```
...
mkdir -p $AUTH_DIR
mkdir -p ~/.kata-agents/workspaces/$WORKSPACE_ID/channels/whatsapp-main

cat > ~/.kata-agents/workspaces/$WORKSPACE_ID/channels/whatsapp-main/config.json << EOF
{
  "slug": "whatsapp-main",
  "adapter": "whatsapp",
  "enabled": true,
