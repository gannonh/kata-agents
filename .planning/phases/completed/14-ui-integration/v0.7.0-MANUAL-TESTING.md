...
Channel configs don't store credentials directly. Instead, they reference a **source** by its slug. A source is a directory under `{rootPath}/sources/` that holds a `config.json` and has an encrypted credential stored in the app's credential manager.

A **source slug** is a URL-safe name you choose when creating the source. It serves as both the directory name and the identifier that channel configs reference. For example, if you name your source `kata-slack-bot`, the channel config references it with `"sourceSlug": "kata-slack-bot"`.

To set this up:

**Step 1:** Pick a slug for your source (e.g., `kata-slack-bot`) and create it:

```bash
WORKSPACE_ROOT="<your-workspace-root-path>"
SOURCE_SLUG="kata-slack-bot"

mkdir -p "$WORKSPACE_ROOT/sources/$SOURCE_SLUG"

cat > "$WORKSPACE_ROOT/sources/$SOURCE_SLUG/config.json" << 'EOF'
{
  "slug": "kata-slack-bot",
  "name": "Slack Bot",
  "type": "api",
...
**Step 2:** Open Kata Agents. In the left sidebar, click the **Sources** icon (the plug icon). "Slack Bot" appears in the sources list panel. Click it to open the source info page, then enter the `xoxb-` bot token in the credentials field. The token is stored encrypted at `~/.kata-agents/credentials.enc`.

When the daemon starts, it reads the channel config, finds `credentials.sourceSlug: "kata-slack-bot"`, looks up the encrypted token for that source, and passes it to the Slack adapter.
...
### 4.2 Create a Source for WhatsApp Auth State

WhatsApp stores authentication state (Signal protocol keys from QR pairing) as files on disk. The source credential holds the path to this directory.

**Step 1:** Create the auth directory and source:
...
mkdir -p "$AUTH_DIR"
mkdir -p "$WORKSPACE_ROOT/sources/my-whatsapp"

cat > "$WORKSPACE_ROOT/sources/my-whatsapp/config.json" << 'EOF'
{
  "slug": "my-whatsapp",
  "name": "WhatsApp",
  "type": "api",
...
**Step 2:** In the app, click the **Sources** icon in the left sidebar. Click "WhatsApp" in the sources list, then enter the auth directory path as the credential: `~/.kata-agents/whatsapp-auth`

### 4.3 Create the WhatsApp Channel Config
...
### 4.6 Enable the WhatsApp Plugin
