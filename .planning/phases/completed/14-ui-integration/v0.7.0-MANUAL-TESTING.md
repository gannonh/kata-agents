...
The channel config's `credentials.sourceSlug` field references a source by its slug. The daemon looks up that source's stored credential (the `xoxb-` bot token) at runtime.

To set this up, create a source config on disk and store the token through the app:

1. Create the source directory and config:
   ```bash
...
The channel config points to this source via `credentials.sourceSlug: "my-slack-bot"`. When the daemon starts, it retrieves the encrypted token and passes it to the Slack adapter.
