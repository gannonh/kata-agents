...
**Step 2:** Open Kata Agents and navigate to Settings for your workspace. The source "Slack Bot" appears in the sources list. Click it, then enter the `xoxb-` bot token in the credentials field. The token is stored encrypted at `~/.kata-agents/credentials.enc`.
...
**Step 2:** In the app, navigate to the source and enter the auth directory path as the credential: `~/.kata-agents/whatsapp-auth`
