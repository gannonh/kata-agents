...
must_haves:
  truths:
    ...
    - "GitWatcher starts within reasonable time for a repository with 1000+ files"
  artifacts:
...
<objective>
Add integration tests for the GitWatcher class covering normal repos, worktree directories, non-git directories, large repo performance, and cleanup behavior.

Purpose: GitWatcher has zero test coverage. These tests catch cross-platform regressions, verify the worktree support added in Plan 01, and validate performance with many files.
...
9. **"starts within reasonable time for a repository with 1000+ files"**
   - Create temp dir, init git repo
   - Generate 1000 files programmatically using a loop: `writeFileSync(join(dir, `file-${i}.txt`), `content ${i}`)` for i in 0..999
   - Run `git add .` + `git commit -m "bulk"` using execSync
   - Record start time with `performance.now()`
   - Create GitWatcher with a mock callback
   - Call start(), expect true
   - Record end time, assert elapsed time < 5000ms (5 seconds is generous; expect well under 1s)
   - Call stop()
   - NOTE: This validates the performance pattern. Full 10k+ file validation is deferred to manual UAT.
...
Run `bun test` to confirm no regressions across the full suite.
  ...
  <done>
GitWatcher has integration tests covering: normal repo start/stop, non-git directory, change detection, worktree support, isRunning state, resource cleanup, and large repo (1000+ files) startup performance. All tests pass.
  </done>
</task>
...
- Test file covers at least 9 distinct scenarios including performance test
- Performance test confirms start() completes in < 5 seconds for 1000+ file repo
</verification>
...
- Large repo performance validated with 1000+ files (full 10k+ deferred to manual UAT)
