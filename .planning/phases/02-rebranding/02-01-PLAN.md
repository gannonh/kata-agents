...
must_haves:
  truths:
    - "Application window title shows 'Kata Agents'"
    - "macOS menu bar shows 'Kata Agents' application menu"
    - "About menu item displays 'About Kata Agents'"
  ...
  key_links:
    - from: "apps/electron/electron-builder.yml"
      ...
      pattern: "productName.*Kata Agents"
---

<objective>
Update all user-facing product names and metadata from "Craft Agents" to "Kata Agents" and change bundle ID to `sh.kata.desktop`.

Purpose: Establishes Kata Agents identity in the most visible places (title bar, menus, system identification) while keeping internal code intact.

Output: Application displays "Kata Agents" everywhere users see the name, and builds produce correctly-identified artifacts.
</objective>
...
2. Line 2: `productName: Craft Agents` -> `productName: Kata Agents`
3. Line 3: Keep copyright as-is (Apache 2.0 requires attribution)
4. Line 72: `artifactName: "Craft-Agent-${arch}.${ext}"` -> `artifactName: "Kata-Agents-${arch}.${ext}"`
5. Line 80: `artifactName: "Craft-Agent-${arch}.dmg"` -> `artifactName: "Kata-Agents-${arch}.dmg"`
6. Line 86: `title: "Craft Agents"` -> `title: "Kata Agents"`
7. Line 105: `artifactName: "Craft-Agent-${arch}.${ext}"` -> `artifactName: "Kata-Agents-${arch}.${ext}"`
8. Line 133: Update maintainer email from `support@craft.do` to a placeholder or remove domain
9. Line 138: `artifactName: "Craft-Agent-${arch}.${ext}"` -> `artifactName: "Kata-Agents-${arch}.${ext}"`
...
# Should show sh.kata.desktop, Kata Agents, Kata-Agents-${arch}
  </verify>
  <done>
electron-builder.yml has bundle ID `sh.kata.desktop`, product name "Kata Agents", and artifact names starting with "Kata-Agents"
  </done>
</task>
...
3. Line ~107: Change app name:
   ...
   ```
4. Update any comments mentioning "Craft Agents" to "Kata Agents"
...
# Should show Kata Agents and kata:// scheme references
  </verify>
  <done>
Main process sets app name to "Kata Agents" and uses `kata://` deeplink scheme by default
  </done>
</task>
...
1. Line 63: `label: 'Craft Agents'` -> `label: 'Kata Agents'`
2. Line 65: `label: 'About Craft Agents'` -> `label: 'About Kata Agents'`
3. Line 74: `label: 'Hide Craft Agents'` -> `label: 'Hide Kata Agents'`
4. Line 78: `label: 'Quit Craft Agents'` -> `label: 'Quit Kata Agents'`
5. Line 185: Update "Craft Agent" in dialog message to "Kata Agents"
6. Line 199: Keep the docs URL for now (handled in later plan)

Update comments:
- Line 11: "Craft Agents app" -> "Kata Agents app"
- Line 25: "Craft logo menu" -> "app logo menu"

In HTML files, update titles:
- index.html line 7: `<title>Craft Agents</title>` -> `<title>Kata Agents</title>`
- playground.html: Same title update if exists
- apps/viewer/index.html: `<title>Craft Agents</title>` -> `<title>Kata Agents</title>`
  </action>
  <verify>
...
# Should return no matches
  ...
  <done>
All menu labels show "Kata Agents" and HTML titles display "Kata Agents"
  </done>
</task>
...
grep "Kata Agents" apps/electron/electron-builder.yml && echo "Product name: OK"
```
...
2. `grep "productName: Kata Agents" apps/electron/electron-builder.yml` returns match
3. `grep -c "Kata Agents" apps/electron/src/main/menu.ts` returns 4+ matches
4. `grep "<title>Kata Agents</title>" apps/electron/src/renderer/index.html` returns match
