...
Purpose: Achieves trademark compliance (BRAND-03) by removing external references to Craft infrastructure, and prepares the distribution mechanism (DIST-01) for the first Kata Agents release.
...
4. **packages/shared/src/prompts/system.ts** - Git co-author:
   - Change "Craft Agent" email to "Kata Agents" or generic
   - `git-co-author@kata.sh` or similar placeholder

5. **apps/electron/src/main/menu.ts** - Help menu:
   - Line 199: Change `https://agents.craft.do/docs` to `https://github.com/gannonh/kata-agents` (GitHub repo as docs for now)
...
2. Update README.md:
   - Change project name to Kata Agents
