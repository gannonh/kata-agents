...
* **Worktree V2 Phase 1** — [#40](https://github.com/gannonh/kata-agents/issues/40) — implementation covers exact named branches, server-owned configurable roots, fixed-registry upgrades, capability-aware Electron controls, and headless/local parity. Local E2E execution remains dependent on the Electron build artifact; see the issue's Verify matrix for current evidence.
