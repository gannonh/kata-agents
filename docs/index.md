...
* **Worktree V2 shipped and enabled by default**: phases 1–4 [#40](https://github.com/gannonh/kata-agents/issues/40), [#41](https://github.com/gannonh/kata-agents/issues/41), [#42](https://github.com/gannonh/kata-agents/issues/42), and [#43](https://github.com/gannonh/kata-agents/issues/43) cover custom identity, server-owned roots, snapshot-backed lifecycle, checkout handoff, and isolated conversation forks. `KATA_FEATURE_WORKTREE_V2=0` remains an explicit V1-only opt-out; provider-specific handoff and native-fork capabilities remain gated by adapter support. See the [Worktree V2 ADRs](adrs/).
...
* **Development and packaged runtime coexistence** — [#34](https://github.com/gannonh/kata-agents/issues/34) — source development uses an isolated `~/.kata-agents-dev` backend/config root and bypasses the production single-instance lock; packaged development uses the same isolated root with a separate Electron instance lock, preserving warm and cold deep-link routing beside installed builds.
