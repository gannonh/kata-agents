...
* **Cross-platform release validation repaired**: the third-party-notices generator now reports actionable normalized diff context, recognizes both OS-specific ExifTool packages before dependency resolution, and retains stable ExifTool runtime attribution, keeping the checked-in inventory identical on macOS, Linux, and Windows. All four build legs across the three operating-system platforms passed in a Nightly dry run after the fix.
