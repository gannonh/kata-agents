...
## How it Works (Video)
To understand what Kata Agents does and how it works watch this video.
...
## Why Kata Agents was built
Kata Agents is a tool we built so that we (at craft.do) can work effectively with agents. It enables intuitive multitasking, no-fluff connection to any API or Service, sharing sessions, and a more document (vs code) centric workflow - in a beautiful and fluid UI.
...
Kata Agents is open source under the Apache 2.0 license - so you are free to remix, change anything. And that's actually possible. We ourselves are building Kata Agents with Kata Agents only - no code editors - so really, any customisation is just a prompt away.

We built Kata Agents because we wanted a better, more opinionated (and preferably non-CLI way) of working with the most powerful agents in the world. We'll continue to improve it, based on our experiences and intuition.
...
Kata Agents connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
Kata Agents can run as a headless server on a remote machine (e.g., a Linux VPS), with the desktop app connecting as a thin client. This lets you keep long-running sessions alive, access them from multiple machines, and run compute-heavy tasks on a powerful server.
...
A terminal client that connects to a running Kata Agent server over WebSocket (`ws://` or `wss://`). Use it for scripting, CI/CD pipelines, server validation, or when you prefer the command line.
...
4. Name: e.g., "Kata Agent Desktop"
5. Click **Create**
6. Note the **Client ID** and **Client Secret**

#### 5. Configure in Kata Agent
...
Kata Agents supports multiple ways to connect to LLM providers:
...
Kata Agents uses two agent backends:
...
**macOS:**
```bash
/Applications/Kata\ Agents.app/Contents/MacOS/Kata\ Agents -- --debug
```

**Windows (PowerShell):**
```powershell
& "$env:LOCALAPPDATA\Programs\@craft-agentelectron\Kata Agents.exe" -- --debug
```
